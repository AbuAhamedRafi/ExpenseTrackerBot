...
def get_category_spending_this_month(category_id, current_month_start, current_month_end):
            ...
            "and": [
                {
                    "property": "Date",
                    "date": {
                        "on_or_after": current_month_start
                    }
                },
                {
                    "property": "Date",
    ...
    if response.status_code == 200:
        ...
        for page in results:
            ...
            if title_list:
                ...
                if expense_name_lower in page_name.lower() or page_name.lower() in expense_name_lower:
                    ...
                    update_response = requests.patch(update_url, json=update_payload, headers=get_headers())
    ...
    if response.status_code == 200:
        ...
        for page in results:
            ...
            if title_list:
                ...
                if item_name_lower in existing_name.lower() or existing_name.lower() in item_name_lower:
    ...
    if response.status_code == 200:
        ...
        for page in results:
            ...
            if title_list:
                ...
                if source_name_lower in existing_name.lower() or existing_name.lower() in source_name_lower:
                    # Get the date of the duplicate
                    date_prop = page.get("properties", {}).get("Date", {})
                    date_info = date_prop.get("date", {})
                    if date_info:
...
def add_expense_to_notion(item, amount, category_name, account_name="BRAC Bank Salary Account"):
    """
    ...
    recurring_keywords = [
        'subscription', 'spotify', 'netflix', 'youtube', 'prime', 'amazon',
        'rent', 'electricity', 'water', 'gas', 'internet', 'wifi', 'broadband',
        'insurance', 'premium', 'emi', 'installment', 'membership', 'gym',
        'phone bill', 'mobile recharge', 'postpaid'
    ]
    ...
    if is_recurring:
        ...
        # Check for duplicate
        duplicate_date = check_duplicate_expense(item, current_month_start, current_month_end)
        if duplicate_date:
    ...
    if response.status_code == 200:
        ...
        if category_id and category_name:
            ...
            if budget:
                ...
                total_spent = get_category_spending_this_month(category_id, current_month_start, current_month_end)
        ...
        return {"success": True, "budget_warning": budget_warning, "checklist_ticked": checklist_message}
    else:
...
def add_income_to_notion(source, amount, account_name="BRAC Bank Salary Account"):
    ...
    # Check for duplicate
    duplicate_date = check_duplicate_income(source, current_month_start, current_month_end)
    if duplicate_date:
        # Parse the date to make it readable
        try:
            date_obj = datetime.fromisoformat(duplicate_date.replace('Z', '+00:00'))
